...
PLL_EXPORT pll_partition_t *pll_partition_create(unsigned int tips,
...
#endif
#ifdef HAVE_AVX512F
  // AVX512 with normal memory layout
  ...
  // AVX512 with SMID memory layout
  if (attributes & PLL_ATTRIB_ARCH_AVX512F && PLL_STAT(avx512f_present)
      && partition->attributes & PLL_ATTRIB_SIMD_MEM_LAYOUT) {
    partition->alignment = PLL_ALIGNMENT_AVX512F;
    ...
    //TODO AVX512F SIMD layout in combination with repeats mode is not supported, fall back to AVX512F without repeats mode
    if (partition->attributes & PLL_ATTRIB_SITE_REPEATS) {
      partition->attributes &= ~PLL_ATTRIB_SITE_REPEATS;
    }
  }
  ...
  unsigned int sites_alloc = partition->asc_additional_sites + partition->sites;
  //TODO (AVX512): Try to move this padding step in vecorization-specific code block above
  if (attributes & PLL_ATTRIB_ARCH_AVX512F && PLL_STAT(avx512f_present)
      && partition->attributes & PLL_ATTRIB_SIMD_MEM_LAYOUT) {
  ...
  if (!pll_repeats_enabled(partition)) {
    ...
    for (i = start; i < partition->tips + partition->clv_buffers; ++i) {
      ...
      memset(partition->clv[i],
             0,
             (size_t) sites_alloc * clv_states_padded * rate_cats * sizeof(double));
    }
  }
  /* pmatrix */
...
static int set_tipclv_sml(pll_partition_t *partition,
                          unsigned int tip_index,
                          const pll_state_t *map,
                          const char *sequence) {
...
static int set_tipclv(pll_partition_t *partition,
                      ...
                      const char *sequence) {
  if (partition->attributes & PLL_ATTRIB_SIMD_MEM_LAYOUT) {
    return set_tipclv_sml(partition, tip_index, map, sequence);
  }
...
//TODO AVX512: Not updated for SIMD layout
PLL_EXPORT int pll_set_tip_clv(pll_partition_t *partition,
  ...
  if (partition->attributes & PLL_ATTRIB_SIMD_MEM_LAYOUT) {
    ...
    snprintf(pll_errmsg, 200,
             "Cannot use pll_set_tip_clv with PLL_ATTRIB_SIMD_MEM_LAYOUT.");
    return PLL_FAILURE;
  }
