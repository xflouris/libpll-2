...
def testFAIL(output):
  error_reason = ''
  with open(output, 'r') as errfile:
      for line in errfile:
          if 'SUMMARY:' in line:
              error_reason += line[len('SUMMARY: '):-1]
  fancyprint("red", "Fail: %s\n" % error_reason)
      ...
      for filename in files:
        ...
        if skipOutput == "":
          ...
          if output == "":
            ...
            call(["mv", "tmp", "result/testfail_"+typestr+"_"+filename+"_"+nowstr])
            call(["mv", "tmperr", "result/testfail_"+typestr+"_"+filename+"_"+nowstr+".err"])
            testFAIL("result/testfail_"+typestr+"_"+filename+"_"+nowstr+".err")
            continue
        ...
        success_count = success_count + result_ok
...
if __name__ == "__main__":
  ...
  if args.lib:
      if not 'LD_LIBRARY_PATH' in os.environ:
        os.environ['LD_LIBRARY_PATH'] = os.path.abspath(args.lib)
      else:
        os.environ['LD_LIBRARY_PATH'] += ":" + os.path.abspath(args.lib)
